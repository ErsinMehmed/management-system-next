...
import OrderSellFilter from "@/components/table/filters/OrderSellFilter";
...
function Filter(props) {
  ...
  return (
    <AnimatePresence>
      {props.show && (
        <motion.div
          className="bg-white rounded-lg shadow border border-gray-100 w-full overflow-hidden"
          key="content"
          initial="collapsed"
          animate="open"
          exit="collapsed"
        ...
        >
          ...
          <div className="sm:grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 pt-4 pb-6">
            {(pathname.includes("/orders") || pathname.includes("/sales")) && (
              <OrderSellFilter data={props.data} setData={props.setData} />
            )}
