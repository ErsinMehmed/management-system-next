...
const Pie = (props) => {
  ...
  if (props.status) {
    ...
    totalCartonCount = props.data.reduce((accumulator, stat) => {
      if (
        (stat.name === "Baking Bad" && stat.weight === 2200) ||
        (stat.name === "Great Whip" && stat.weight === 2200)
      ) {
        accumulator += stat.total_quantity / 4;
      } else if (stat.name === "Great Whip" && stat.weight === 2200) {
        accumulator += stat.total_quantity / 2;
      } else {
  ...
  const chartOptions = {
    series: seriesData,
    colors: ["#38bdf8", "#60a5fa", "#fbbf24", "#34d399", "#c084fc", "#fb7185"],
    chart: {
      height: 420,
      width: "100%",
      type: "pie",
