...
const IncomesClient = ({ initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      incomeStore.hydrateAllIncomes(initialData.allIncomes);
      userStore.hydrateDistributors(initialData.distributors);
      return;
    }
    loadAllIncomes();
    loadDistributors();
...
export default observer(IncomesClient);
